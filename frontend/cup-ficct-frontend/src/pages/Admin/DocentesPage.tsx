...
// Paginación igual que PostulanteListPage. Los docentes con cuenta
// de usuario pueden iniciar sesión en el sistema con rol 'docente'.

import { useEffect, useState } from "react";
import {
  Plus, Search, Pencil, UserX, UserCheck, GraduationCap,
  BadgeCheck, ChevronLeft, ChevronRight,
} from "lucide-react";
import { toast } from "sonner";
import { useConfirm } from "@/context/useConfirm";
import {
  docentesService, type Docente, type DocenteInput, type GradoAcademico,
...
const formVacio: DocenteInput = {
  ci: "", nombres: "", apellidos: "", email: "", telefono: "", titulo: "",
  grado_academico: "Licenciatura", diplomado_docencia: false, crear_cuenta: false, password: "",
};

// ── Modal crear/editar ────────────────────────────────────────────────────────
function DocenteModal({ open, docente, onClose, onSaved }: {
  open: boolean; docente: Docente | null; onClose: () => void; onSaved: () => void;
}) {
  ...
  useEffect(() => {
    if (docente) {
      setForm({
        ci: docente.ci, nombres: docente.nombres, apellidos: docente.apellidos,
        email: docente.email, telefono: docente.telefono ?? "", titulo: docente.titulo ?? "",
        grado_academico: docente.grado_academico, diplomado_docencia: docente.diplomado_docencia,
        crear_cuenta: false, password: "",
      });
    } else {
      setForm(formVacio);
    }
    ...
    try {
      if (esEdicion && docente) {
        await docentesService.update(docente.id, {
          ...
          email: form.email, telefono: form.telefono, titulo: form.titulo,
          grado_academico: form.grado_academico, diplomado_docencia: form.diplomado_docencia,
        });
      ...
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid gap-4 py-2">
          <div className="grid gap-1.5">
            ...
            <Input id="d-ci" value={form.ci} onChange={(e) => set("ci", e.target.value)}
              placeholder="Ej: 7654321" className={errors.ci ? "border-destructive" : ""} />
            {errors.ci && <p className="text-xs text-destructive">{errors.ci}</p>}
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="grid gap-1.5">
              <Label htmlFor="d-nombres">Nombres</Label>
              <Input id="d-nombres" value={form.nombres} onChange={(e) => set("nombres", e.target.value)}
            ...
            <div className="grid gap-1.5">
              ...
              <Input id="d-apellidos" value={form.apellidos} onChange={(e) => set("apellidos", e.target.value)}
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="grid gap-1.5">
              ...
              <Input id="d-telefono" value={form.telefono} onChange={(e) => set("telefono", e.target.value)}
                placeholder="Opcional" />
            </div>
          </div>
          <div className="grid gap-1.5">
            <Label htmlFor="d-titulo">Título profesional</Label>
            <Input id="d-titulo" value={form.titulo} onChange={(e) => set("titulo", e.target.value)}
              placeholder="Ej: Lic. en Matemáticas" />
          </div>
          <div className="grid gap-1.5">
            <Label>Grado académico</Label>
            <Select value={form.grado_academico} onValueChange={(v) => set("grado_academico", v as GradoAcademico)}>
          ...
          <div className="flex items-center gap-2">
            <Checkbox id="d-diplomado" checked={form.diplomado_docencia}
          ...
          {!esEdicion && (
            <div className="rounded-lg border border-dashed border-border p-4 space-y-3">
              <div className="flex items-center gap-2">
                <Checkbox id="d-crear-cuenta" checked={form.crear_cuenta}
                  onCheckedChange={(v: boolean | "indeterminate") => set("crear_cuenta", Boolean(v))} />
              ...
              {form.crear_cuenta && (
                <div className="grid gap-1.5">
                  ...
                  <Input id="d-password" type="password" value={form.password}
...
export function DocentesPage() {
  ...
  async function cargar(p: number = page) {
    setLoading(true);
    try {
      const params: { search?: string; activo?: boolean; page?: number } = { page: p };
  ...
  useEffect(() => { cargar(1); }, [filtroActivo]);
  ...
  async function toggleActivo(d: Docente) {
    ...
    const ok = await confirm({
      title: `${verbo} docente`,
      description: `¿Está seguro de ${accion} a ${nombre}?${d.activo && d.user_id ? " También se desactivará su cuenta de acceso." : ""}`,
      confirmText: verbo,
  ...
  return (
    <div className="space-y-6 p-6">
      <PageHeader
        title="Docentes"
        description={total > 0 ? `${total} docente${total === 1 ? "" : "s"} registrados` : "Registro de docentes del CUP-FICCT."}
      >
      ...
      <ContentCard className="p-4">
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <Input placeholder="Buscar por nombre, CI o correo..." value={search}
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                docentes.map((d) => (
                  <TableRow key={d.id} className={cn("transition-colors hover:bg-muted/30", !d.activo && "opacity-60")}>
                    <TableCell>
                      <div>
                        <p className="font-medium">{d.nombres} {d.apellidos}</p>
                        <p className="text-xs text-muted-foreground">{d.email}</p>
                        {d.titulo && <p className="text-xs text-muted-foreground italic">{d.titulo}</p>}
                      </div>
                    </TableCell>
                    ...
                    <TableCell>
                      <span className={cn("inline-flex items-center gap-1 rounded-full border px-2.5 py-0.5 text-xs font-medium",
                        ...
                        <GraduationCap className="h-3 w-3" />{d.grado_academico}
                      </span>
                    </TableCell>
                    <TableCell>
                      {d.diplomado_docencia ? (
                        <span className="inline-flex items-center gap-1 text-xs text-emerald-600 dark:text-emerald-400">
                          <BadgeCheck className="h-3.5 w-3.5" />Sí
                        </span>
                      ) : (
                    ...
                    <TableCell>
                      <span className={cn("inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-medium",
                        d.activo ? "bg-emerald-500/10 text-emerald-600 border-emerald-500/20 dark:text-emerald-400"
                    ...
                    <TableCell>
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <Button variant="ghost" size="sm" onClick={() => toggleActivo(d)}
                          className={cn("h-8 w-8 p-0", d.activo
                            ? "hover:bg-destructive/10 hover:text-destructive"
                            : "hover:bg-emerald-500/10 hover:text-emerald-600")}
                          title={d.activo ? "Desactivar" : "Reactivar"}>
                          {d.activo ? <UserX className="h-3.5 w-3.5" /> : <UserCheck className="h-3.5 w-3.5" />}
                        </Button>
                      </div>
                    </TableCell>
                  </TableRow>
      ...
      {!loading && total > 0 && (
        <div className="flex flex-wrap items-center justify-between gap-3">
          <p className="text-sm text-muted-foreground">
            ...
            <span className="font-medium text-foreground">{lastPage}</span> · {total} docente{total === 1 ? "" : "s"}
          </p>
      ...
      <DocenteModal open={modalOpen} docente={docenteEditar}
